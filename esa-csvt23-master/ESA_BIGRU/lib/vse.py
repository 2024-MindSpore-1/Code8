...
def mindspore2torch(value_mindspore):
    value = torch.from_numpy(value_mindspore.asnumpy())
    return value
...
class VSEModel(object):
    ...
    def forward_emb_mindspore(self, model_ESA, images, captions, lengths, image_lengths=None):
        """Compute the image and caption embeddings
        """
        # Set mini-batch dataset
        if torch.cuda.is_available():
            ...
            captions = captions.cuda()
            image_lengths = image_lengths.cuda()
        img_emb = self.img_enc.forward_img_emb(images, image_lengths)

        lengths = torch.Tensor(lengths).cuda()
        cap_emb, mask = self.txt_enc.forward_cap_emb(captions, lengths)

        #转为mindspore
        img_emb_mindspore=torch2mindspore(img_emb)
        cap_emb_mindspore = torch2mindspore(cap_emb)
        mask_mindspore = torch2mindspore(mask)

        #送入ESA
        img_feature_mindspore, cap_feature_mindspore =model_ESA(img_emb_mindspore,cap_emb_mindspore,mask_mindspore)

        return img_feature_mindspore, cap_feature_mindspore
