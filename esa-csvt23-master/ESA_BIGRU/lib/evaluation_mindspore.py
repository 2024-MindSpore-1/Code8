...
def encode_data_mindspore(model, model_ESA, data_loader, log_step=10, logging=logger.info):
    """Encode all images and captions loadable by `data_loader`
    """
    batch_time = AverageMeter()
    val_logger = LogCollector()
...
class ESAModel(mindspore_nn.Cell):
    ...
    def load_weights(self, weights):
        ...
        params_img_dict ={'linear_img.weight': Parameter(torch2mindspore(weights[0]['linear1.weight']),requires_grad=False),
                            'linear_img.bias': Parameter(torch2mindspore(weights[0]['linear1.bias']),requires_grad=False)}

        params_txt_dict ={'linear_txt.weight': Parameter(torch2mindspore(weights[1]['linear1.weight']),requires_grad=False),
                            'linear_txt.bias': Parameter(torch2mindspore(weights[1]['linear1.bias']),requires_grad=False)}
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None):
    ...
    opt.vocab_path = '/opt/data/private/kevin/data/vocab'
    ...
    with torch.no_grad():
        ...
        img_embs, cap_embs = encode_data_mindspore(model, model_ESA, data_loader)#提特征
