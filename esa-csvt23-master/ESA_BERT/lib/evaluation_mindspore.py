...
import mindspore.nn as mindspore_nn
...
def encode_data_mindspore(model, model_ESA, data_loader, log_step=10, logging=logger.info, backbone=False):
    """Encode all images and captions loadable by `data_loader`
    """
    batch_time = AverageMeter()
    val_logger = LogCollector()
    ...
    for i, data_i in enumerate(data_loader):
        ...
        # compute the embeddings
        if not backbone:
            img_emb, cap_emb = model.forward_emb_mindspore(model_ESA, images, captions, lengths, image_lengths=image_lengths)
        else:
            img_emb, cap_emb = model.forward_emb_mindspore(model_ESA, images, captions, lengths)

        if img_embs is None:
            if img_emb.ndim == 3:
                img_embs = np.zeros((len(data_loader.dataset), img_emb.shape[1], img_emb.shape[2]))
            else:
                img_embs = np.zeros((len(data_loader.dataset), img_emb.shape[1]))
            cap_embs = np.zeros((len(data_loader.dataset), cap_emb.shape[1]))
            cap_lens = [0] * len(data_loader.dataset)
        # cache embeddings
        img_embs[ids] = img_emb.asnumpy()
        cap_embs[ids, :] = cap_emb.asnumpy()
...
def torch2mindspore(value):
    value_mindspore = ms.Tensor.from_numpy(value.cpu().numpy())
    return value_mindspore

class ESAModel(mindspore_nn.Cell):
    """
        The Mindspore implement of ESA model
    """
    def __init__(self, opt):
        # Build Models
        super().__init__()
        self.grad_clip = opt.grad_clip
        self.linear_img = mindspore_nn.Dense(opt.embed_size, opt.embed_size)
        self.linear_txt = mindspore_nn.Dense(opt.embed_size, opt.embed_size)

    def construct(self, img_emb, cap_emb, cap_mask, norm=True):
        img_external = self.linear_img(img_emb)
        attn_img = mindspore_nn.Softmax(axis=1)(img_external-ops.unsqueeze(ops.max(img_external,axis=1)[0], dim=1))
        feature_img = ops.sum(attn_img * img_emb,dim=1)

        cap_external = self.linear_txt(cap_emb)
        cap_external = ops.masked_fill(cap_external,cap_mask == 0,-10000)
        attn_cap = mindspore_nn.Softmax(axis=1)(cap_external-ops.unsqueeze(ops.max(img_external,axis=1)[0], dim=1))
        attn_cap = ops.masked_fill(attn_cap,cap_mask == 0,0)
        feature_cap = ops.sum(attn_cap * cap_emb,dim=1)

        if norm:
            l2norm = ops.L2Normalize(axis=-1,epsilon=1e-12)
            feature_img = l2norm(feature_img)
            feature_cap = l2norm(feature_cap)

        return feature_img, feature_cap
    
    def load_weights(self, weights):
        from mindspore import Tensor, Parameter
        params_img_dict ={'linear_img.weight': Parameter(torch2mindspore(weights[0]['module.linear1.weight']),requires_grad=False),
                            'linear_img.bias': Parameter(torch2mindspore(weights[0]['module.linear1.bias']),requires_grad=False)}

        params_txt_dict ={'linear_txt.weight': Parameter(torch2mindspore(weights[1]['module.linear1.weight']),requires_grad=False),
                            'linear_txt.bias': Parameter(torch2mindspore(weights[1]['module.linear1.bias']),requires_grad=False)}
        ms.load_param_into_net(self.linear_img, params_img_dict)
        ms.load_param_into_net(self.linear_txt, params_txt_dict)
        return
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    tokenizer = BertTokenizer.from_pretrained('/root/.cache/huggingface/hub/bert-base-uncased/')
    vocab = tokenizer.vocab
    opt.vocab_size = len(vocab)

    opt.backbone_path = '/opt/data/private/kevin/data/weights/original_updown_backbone.pth'
    if data_path is not None:
        opt.data_path = data_path

    # construct model
    model = VSEModel(opt)
    model_ESA = ESAModel(opt)
    ...
    model_ESA.load_weights(checkpoint['model'])
    model_ESA.set_train(False)
    ...
    with torch.no_grad():
        if opt.precomp_enc_type == 'basic':
            img_embs, cap_embs = encode_data_mindspore(model, model_ESA, data_loader)
        else:
